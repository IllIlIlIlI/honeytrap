...
var mapRequests = map[string]createMsg{
	...
	"op_query":         (*mongodbService).isMasterMsg,
	//...
}

func (s *mongodbService) encode(reqid int32, port string, rq string) []byte {

	kv = 48
	fmt.Println("kv:", kv)
	src := []byte{}

	reqID := encodeInt32(reqid + 1)
	respID := encodeInt32(reqid)
...
func localTime() string {
	...
	local := make([]byte, 8)
	binary.LittleEndian.PutUint64(local, uint64(t))
	l := make([]byte, hex.EncodedLen(len(local)))
	hex.Encode(l, local)
	return string(l)
}

func encodeInt32(t int32) string {
	local := make([]byte, 4)
	binary.LittleEndian.PutUint32(local, uint32(t))
	l := make([]byte, hex.EncodedLen(len(local)))
	hex.Encode(l, local)
	return string(l)
}
...
func (s *mongodbService) isMasterMsg(arg []string) []byte {
	...
	lt := localTime()
	l := "ef000000"
	p1 := "010000000800000000000000000000000000000001000000cb0000000869736d" +
	...
	p2 := "106c6f676963616c53657373696f6e54696d656f75744d696e75746573001e00" +
	...
	return []byte(l + arg[0] + arg[1] + p1 + lt + p2)
}

func (s *mongodbService) uriMsg(args []string) []byte {
	src := []byte(args[2])
	p := make([]byte, hex.EncodedLen(len(src)))
	hex.Encode(p, src)

	l := "3f000000"
	p1 := "dd07000000000000002a00000002796f7500100000003132372e302e302e313a"
	p2 := "00016f6b00000000000000f03f00"

	return []byte(l + args[0] + args[1] + p1 + string(p) + p2)
}

func (s *mongodbService) forShellMsg(arg []string) []byte {
	l := "e0000000"
	lt := localTime()
	p1 := "dd0700000000000000cb0000000869736d61737465720001106d617842736f6e4" +
	...
	p2 := "106c6f676963616c53657373696f6e54696d656f75744d696e75746573001e000" +
...
func keyValue(kv int) string {
	return fmt.Sprintf("%x", kv)
}

func parseDB(d Db) (int32, string, int64) {
	headerDB := documentBS + keyValue(kv) + zeroBS
	nameDB := stringBS + "6e616d65" + zeroBS + encodeInt32(int32(len(d.Name)+1)) + hex.EncodeToString([]byte(d.Name)) + zeroBS
	...
	s := headerDB + encodeInt32(int32(lengthDB)) + nameDB + sizeDB + emptyDB + zeroBS
...
func (s *mongodbService) listDatabases(arg []string) []byte {
	...
	sizeArrayTotal := encodeInt32(int32(len(arrayTotal)/2+len("aabbccdd")/2) + 1)
	sectionArray := arrayBS + docName + zeroBS + sizeArrayTotal + arrayTotal

	// sectionTotalSize
	totalSizeOnDiskField := "746f74616c53697a65" // "totalSize"
	sectionTotalSize := zeroBS + doubleBS + totalSizeOnDiskField + zeroBS + dbSize(totalSizeOnDiskValue)
	...
	// Total
	opCode := encodeInt32(int32(2013))
	flagsBits := encodeInt32(int32(0))

	sections := sectionArray + sectionTotalSize + sectionOk
	lengthDiffSections := encodeInt32(int32(len(sections)/2 + len("aabbccdd")/2))

	object := arg[0] + arg[1] + opCode + zeroBS + flagsBits + lengthDiffSections + sections

	lengthObject := encodeInt32(int32(len(object)/2 + len("aabbccdd")/2))
...
func (s *mongodbService) buildInfoMsg(arg []string) []byte {
	...
	for _, s := range strings.Split(s.Version, ".") {
		i, _ := strconv.Atoi(s)
		vs = append(vs, encodeInt32(int32(i)))
	}
	m1 := "dd0700000000000000bf0500000276657273696f6e0006000000"
	m2 := "000267697456657273696f6e0029000000393538366535353764353465663730" +
	...
	m3 := "103100"
	m4 := "103200"
	m5 := "1033000000000000036f70656e73736c00570000000272756e6e696e67001c0000004f70656e53534c20312e3" +
	...
	return []byte(l + arg[0] + arg[1] + m1 + v + m2 + vs[0] + m3 + vs[1] + m4 + vs[2] + m5)
}

func (s *mongodbService) getLogMsg(arg []string) []byte {
	l := "ce010000"
	m := "dd0700000000000000b901000010746f74616c4c696e65735772697474656e000" +
	...
	return []byte(l + arg[0] + arg[1] + m)
}

func (s *mongodbService) replSetMsg(arg []string) []byte {
	l := "7a000000"
	m := "dd070000000000000065000000016f6b000000000000000000026572726d73670" +
